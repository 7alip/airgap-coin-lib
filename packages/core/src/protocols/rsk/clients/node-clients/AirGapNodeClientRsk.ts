...
export class AirGapNodeClientRsk extends RskNodeClient {
  ...
  private async send(body: RskRPCBody): Promise<RskRPCResponse> {
    const response = await axios.post(this.baseURL, body.toRPCBody(), { headers: { 'Content-Type': 'application/json' } }).catch((error) => {
  ...
  private async batchSend(bodies: RskRPCBody[]): Promise<RskRPCResponse[]> {
    const data = (await axios.post(this.baseURL, JSON.stringify(bodies.map((body) => body.toJSON())), { headers: { 'Content-Type': 'application/json' } })).data
    return data
  }
}
